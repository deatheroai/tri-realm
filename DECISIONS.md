...
- **2026-09-09 — Swim clips in Air read as "a fish," not the "balloon" feel
  ...
  seeing an actual animation instead of running, but the active swim-stroke
  clip playing while moving looked like deliberate paddling, not drifting.
  Fixed directly in this session: air now always uses the calm `swimIdle`
  clip for a swim-capable skin, regardless of speed or direction — never
  the active stroke sea itself still uses. Sea untouched. Verified visually
  (screenshot) and with 3 new unit tests plus updated E2E coverage; full
  suite green. **Review checkpoint: confirmed — "Like balloons than fish."**
  See `BACKLOG.md` for the full writeup.
